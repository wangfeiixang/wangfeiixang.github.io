    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.resultCode == "OK") {
                ...
                console.log('数据接收成功--竹子数量' + turntable.getBamboo, 'data--', data)
...
var prize = {
    ...
    scroll: function () { //开启字幕滚动
        ...
        $($(this.ali)[this.index]).addClass("active").animate({
            top: "-25px"
        }, 1000, "linear").siblings("li").removeClass("active").css({
            top: "25px",
            opacity: "0"
        });

        if (this.index === this.ali.length - 1) {
            $($(this.ali)[0]).css("opacity", "1").animate({
                top: "0px"
            }, 1000, "linear");
        } else {
            $($(this.ali)[this.index + 1]).css("opacity", "1").animate({
                top: "0px"
            }, 1000, "linear");
        }
